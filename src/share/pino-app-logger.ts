...
export type AppLogLevel = Exclude<
  keyof AppLog,
  "flushSync" | "untilReady" | "on"
>;
...
export function createAppLog({
  ...
  destination.on("ready", () => {
    ...
    destination.on("error", (error: unknown) =>
      console.error("[Error in Pino]", error)
    );
    resolve();
  });
      ...
      if (ready) logger[level](textMessage);
        ...
        if (!waitingMessages) {
          ...
          void untilReady.then(() => {
            if (waitingMessages) {
              if (file)
                waitingMessages.forEach((wMessages) =>
                  logger[level](messagesToString(wMessages))
                );
  ...
  console.log(
    `Application log with level '${level}' is in: ${file ?? "stdout"}.`
  );
  ...
  switch (typeof msg) {
    ...
    case "object":
      ...
      if (Array.isArray(msg))
        return `[${msg
          .map((child) => messageToString(child, [...parents, msg]))
          .join(",")}]`;
      ...
      return `{${Object.entries(msg)
        .map(
          ([key, child]) =>
            `${key}: ${messageToString(child, [...parents, msg])}`
        )
        .join(",")}}`;
    default:
